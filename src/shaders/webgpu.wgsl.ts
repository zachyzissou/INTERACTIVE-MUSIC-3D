...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let color1 = vec3(0.2, 0.8, 1.0) * (1.0 + uniforms.bassLevel);
    let color2 = vec3(1.0, 0.3, 0.8) * (1.0 + uniforms.highLevel);
    ...
    finalColor += vec3(0.0, 0.8, 1.0) * glow * uniforms.midLevel * 0.5;
    
    // Audio-reactive edge enhancement
    let edge = fwidth(field) * 10.0;
    finalColor += vec3(1.0, 0.2, 0.8) * edge * uniforms.highLevel;
...
fn cs_main(@builtin(global_invocation_id) globalId: vec3<u32>) {
    ...
    let audioForce = vec2(
        sin(time + f32(index) * 0.1) * audioData.bass * 0.01,
        cos(time + f32(index) * 0.15) * audioData.mid * 0.01
    );
    
    particle.velocity += audioForce;
    ...
    particle.size = 0.005 + audioData.spectrum[index % 16] * 0.02;
